...
1. NWB format:
    ...
    Note that validation is also done during `upload`, but ensuring compliance using `validate` prior upload helps avoid interruptions of the lenthier upload process due to validation failures.

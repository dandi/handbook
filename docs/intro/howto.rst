...
TODO: reposition since dandi client specific

DANDI client (`dandi`) uses command-line arguments in a *terminal*. This means that there is no graphical
user interface with buttons to click on, but a set of commands and options users type into
their shell. If you are not used to working with command-line arguments, `dandi` can
...
non-DANDI command or principle you want to remind yourself of.
If you are already familiar with the shell and know the difference between an absolute
and a relative path, you can safely skip this chapter and continue to the DANDI Basics.

Almost all of this chapter is borrowed from the excellent `DataLad Handbook <http://handbook.datalad.org>`_,
this chapter of which is based on parts of a wonderful lab documentation
Alex Waite wrote.
...
`dandi` commands naturally also come with many options, and in the next chapters
...
.. code-block:: bash
   ...
   $ mkdir /home/adina/awesome_project

And that works. ``/home/adina/awesome_project`` is what is called an *absolute*
...
``cd /home/adina/awesome_project/aligned/code``
...
second ``../`` to ``awesome_project/``, and the last ``../``
...
**That's it -- equipped with the basics of Unix, you are good to go on your adventure with DANDI client!**
